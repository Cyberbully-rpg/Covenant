...
### 6D.5 The four remaining levers, all measured

Everything §6D.4 listed as untested was run on the same 102 held-out contracts / 1,377 rows against the same control. One was adopted; three were rejected on evidence. Predictions are recorded next to outcomes, including where they were wrong.

| lever | predicted | measured | cost | verdict |
|---|---|---|---|---|
| Query boilerplate stripping | unknown | **+0.0109** (0.8351 → 0.8460) | **−22% runtime** | **adopted** |
| Character n-grams ("fuzzy search") | +1.0 to +1.5 | +0.0015 | 2× runtime | rejected |
| Cross-encoder rerank, top 20 | +3 to +6 | **−0.0443** | 12× runtime | rejected |
| Retrieval-tuned embedding model | unknown | −0.0226 dense-alone | 3× ingest | rejected |

**Query stripping (adopted, TRD §7.4).** Every probe carries a byte-identical instruction wrapper that is most of its tokens; `retrieval/query.py` reduces it to `<category>. <details>`. Uniquely among everything measured in Phase 6, **all four metrics improved at once and cost fell**: hit_rate +0.0109, gold_recall +0.0445, gold_density +0.0031 (*up* — no volume in the gain), retrieved chars down, runtime down 22%. The hit_rate delta alone is ~1 standard error; the simultaneous movement of four independent metrics at lower cost is what justifies adoption, not the headline.

**Character n-grams (rejected).** The motivating measurement held up — 62.8% of Exclusivity and 25.2% of Non-Compete gold spans contain only a *stem* of the query's words, which word-level TF-IDF treats as unrelated tokens. But converting them requires the char ranker to push those segments into the top 5, and the noise it adds elsewhere cancels the gain. **The +1.0–1.5 prediction was wrong by an order of magnitude.** `gold_recall` did rise +0.033 — it retrieves more gold text without flipping more rows into hits. Kept in `hybrid.py` behind `w_char=0.0`.

**Cross-encoder rerank (rejected).** The headroom it targets is real: the answer is in the top 20 for 92.3% of questions. This reranker cannot convert it — it consistently prefers short passages (retrieved chars fall to 9,047, gold_density rises to 0.0355), which is the wrong instinct for CUAD, where gold spans often sit inside long clauses. Its first measurement was worse still (0.65) because it was fed the raw instruction template; query stripping recovered ~+0.12, so that confound was real and the remaining deficit is the model itself. At ~901ms/query vs ~60ms, it is also the only variant a user would feel.

**BGE-small (rejected, TRD §7.1 rung 3).** A model trained specifically for asymmetric query→passage retrieval, with a 512-token window (1,740-char windows, 28,477 records vs MiniLM's 45,254) and its instruction prefix correctly applied to the query side only — handled centrally in `embedding.py` and asserted by test, since omitting it degrades silently and would have produced a false negative. **Dense-alone it scored 0.6107 against MiniLM's 0.6333 — worse.** In the full stack, 0.8504 vs 0.8460, a +0.0044 difference against a ~0.010 standard error, for 3× the ingestion cost. Adopting on that margin is selection noise. Note this also weakens rung 4: quadrupling context per vector made things worse, which is evidence against window size being the remaining limit.

**Final held-out configuration: `hybrid_bigram_prior_cleanq` at 0.8460 hit_rate@5**, against 0.6934 for the Phase 3 TF-IDF baseline and a 0.9985 segmentation ceiling.

The pattern across all four is consistent with §6D.4's diagnosis and worth stating plainly: **better general-purpose retrieval machinery is not what this task is short of.** The two changes that produced real gains — the positional prior and query stripping — both worked by giving the ranker *the right input*, not by making the ranker stronger. The dominant remaining failure mode is categories whose answer shares no vocabulary with the question, which is a classification problem and belongs to Phase 8.

### 6D.6 Tests

`test_embedding.py` (determinism, L2-normalization, similarity ordering), `test_ingest.py` (metadata correctness, the no-classifier-import boundary, idempotent reset, window coverage/overlap, parent-span collapse, per-model collection namespacing), `test_retrieve.py` (per-contract scoping — a query never returns another contract's spans — interface-shape compatibility with `scorer.score_query()`, window dedup, and batched-equals-unbatched), `test_hybrid.py` (RRF arithmetic against stubbed rankers: the agreement bonus, exact degradation to either single ranker at zero weight, depth limiting, k capping), `test_lead_prior.py` (the prior fires only for fitted categories and is otherwise the identity function, rejects categories that only sometimes lead or have too few rows to judge, and learns only from the rows it is handed). `test_query.py` (stripping keeps the category and details, drops the constant wrapper, no-ops on free text, and sits below the prior so the prior still sees the original probe), `test_rerank.py` (candidates pulled at candidate_depth not k, and scores attributed to the right question/span pair when a whole contract is batched into one predict call). 60 tests in `backend/rag`, **122 tests total**.
