...
**Asymmetric query prefixes are handled centrally.** BGE-family models are trained with a retrieval instruction on the **query side only**; omitting it costs accuracy silently rather than raising. `embed(..., is_query=True)` applies it, ingestion never sets it, and a test asserts `retrieve.py` actually passes it — this is the same silent-misuse shape as the truncation confound (§6D.3), so it is enforced rather than remembered.

**Escalation ladder (priority order, evidence-gated).** Each rung is adopted only if it beats the incumbent at equal k, judged on hit_rate@5 **read alongside** gold_density — a hit_rate gain bought purely with more retrieved characters is not a gain (§3.3). Anything fitted is judged on `--split test`.
1. Remove the truncation confound (windowed ingestion) — **done, adopted** (+0.0804 on the dense path).
2. Hybrid lexical+dense fusion (§7.2) — **done, adopted** (argued; see §7.2).
3. A retrieval-tuned general model — **tested, REJECTED.** `BAAI/bge-small-en-v1.5` (33M, 512-token window, instruction-prefixed) scored **0.6107 dense-alone on held-out vs MiniLM's 0.6333** — worse, despite being trained for asymmetric retrieval and covering twice the text per vector (28,477 records vs 45,254). In the full stack it landed at 0.8504 vs MiniLM's 0.8460, a +0.0044 difference against a ~0.010 standard error, for roughly 3x the ingestion cost. Adopting on that margin would be selection noise. **MiniLM stays.** The result is consistent with everything else measured here: the failures are not caused by weak general embeddings (see §7.4 and ARCHITECTURE.md §6D.4).
4. A long-context embedding model, if evidence points at window size rather than model capacity — **weak prior now.** Rung 3 already quadrupled effective context per vector and got worse, which is evidence against window size being the limit.
...
### 7.4 Query Boilerplate Stripping (locked)

- **What it does:** every CUAD probe is wrapped in a byte-identical instruction template (`Highlight the parts (if any) of this contract related to "X" that should be reviewed by a lawyer. Details: ...`). `retrieval/query.py` reduces it to `<category>. <details>`, keeping every informative token and dropping the constant. It is a no-op on any string that doesn't match the template, so free-text `/ask` questions pass through untouched.
- **Measured on 102 held-out contracts: 0.8351 -> 0.8460 hit_rate@5.** Adopted because *every* metric moved the right way at once and cost fell: gold_recall +0.0445, gold_density +0.0031 (**up**, so no volume in the gain), mean retrieved chars **down**, runtime **down 22%**. The hit_rate delta alone is only ~1 standard error; four independent metrics improving simultaneously while cost drops is what justifies adoption, not the headline number.
- **Placement is load-bearing:** `CleanQueryRetriever` sits strictly BELOW the §7.3 prior in the stack, because the prior parses the category out of the *original* probe text. Cleaning above it would silently disable the prior. Asserted by test.
- **Not label use:** the category is stated in the question itself and gold spans are untouched. It does lean on CUAD's templated format, same caveat as §7.3.

### 7.5 Rejected Retrieval Escalations (measured, not assumed)

Both were run on the full held-out split against the same control, and both are recorded here so they are not re-proposed on intuition.

- **Character n-gram fusion ("fuzzy search") — REJECTED.** `analyzer="char_wb"`, ngrams (3,5), added as a third equal-weight RRF ranker. **+0.0015** (0.8351 -> 0.8366) at **2x runtime**. The motivating measurement was real — 62.8% of Exclusivity and 25.2% of Non-Compete gold spans contain only a *stem* of the query's words — but converting those requires the char ranker to push them into the top 5, and the noise it adds elsewhere cancels the gain. `gold_recall` did rise (+0.033), i.e. it retrieves more gold text without flipping more rows into hits. The capability is kept in `hybrid.py` behind `w_char=0.0`, off by default.
- **Cross-encoder reranking — REJECTED.** `ms-marco-MiniLM-L-6-v2` over the top 20. **-0.0443** (0.8460 -> 0.7908) at **12x runtime** (~901ms/query vs ~60ms). It consistently prefers short passages — retrieved chars fall to 9,047 while gold_density rises to 0.0355 — which is the wrong instinct for CUAD, where gold spans often sit inside long clauses. Note the first measurement was worse still (0.65) because it was fed the raw instruction template; §7.4's stripping recovered ~+0.12 of that, so the confound was real and the remaining deficit is the model. **The headroom it was meant to convert is real** (the answer is in the top 20 for 92.3% of questions, ARCHITECTURE.md §6D.4) — this particular reranker simply cannot convert it. Reopening condition: a reranker not trained on short web passages, or one fine-tuned on contract text.
