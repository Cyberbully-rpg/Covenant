...
SCHEME_PRIORITY = ["article", "section_nn", "bare_nn"]
...
def sub_split_oversized(segment_text: str) -> list[tuple[int, int, str]]:
    ...
    within it. Tries section_nn then bare_nn (article-level numbering
    shouldn't recur inside an article). Falls back to no split if neither
    pattern clears the threshold inside this segment.

    Returns list of (start, end, header) offsets RELATIVE to segment_text.
    """
    for scheme in ("section_nn", "bare_nn"):
...
def segment_contract(raw_text: str, doc_id: str = "doc") -> list[Segment]:
    ...
    if scheme is None:
        # fallback: whole document is one segment
        text = cleaned_text
        segments.append(Segment(
            segment_id=f"{doc_id}_seg0",
            text=text,
            embedding_text=text,
            start_char=cleaned_to_original_offset(0, offset_map),
            end_char=cleaned_to_original_end_offset(len(cleaned_text), offset_map),
            scheme="fallback",
            header="",
        ))
        return segments
